...
import { Card, Button, Modal, LoadingSpinner, EmptyState, Badge, Input, Skeleton } from '../components/UIComponents';
...
const AllServicesPage = () => {
    ...
    const initializePage = async () => {
        try {
            const servicesData = await servicesAPI.getAllDetailed();
    ...
    if (loading || !user) {
        return (
            <div className="container">
                <div className="mb-10">
                    <Skeleton height={48} width="40%" className="mb-2" />
                    <Skeleton height={24} width="60%" />
                </div>

                <div className="mb-8 flex flex-wrap gap-2">
                    {[1, 2, 3, 4, 5].map((i) => (
                        <Skeleton key={i} height={36} width={80} className="rounded-lg" />
                    ))}
                </div>
